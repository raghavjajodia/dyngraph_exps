...
parser.add_argument("--node-dim", type=int, default=64, help="node dim")
...
max_out_degrees = max(map(lambda grph: torch.max(grph.out_degrees()).item(), train_graphs + valid_graphs + test_graphs))
...
# create GCN model
model = GCN(max_out_degrees, node_dim, hid_dim, mlp_dim, n_layers, F.relu, dropout)
model.to(device)
